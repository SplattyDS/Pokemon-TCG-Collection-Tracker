<?php
$FUT_SM = array('SM1','SM2','SM3','SM4','SM5','SM6','SM7','SM8','SM9','SM10','SM11','SM12','SM13','SM14','SM15','SM16','SM17','SM18','SM19','SM20','SM21','SM22','SM23','SM24','SM25','SM26','SM27','SM28','SM29','SM30','SM31','SM32','SM33','SM34','SM35','SM36','SM37','SM38','SM39','SM40','SM41','SM42','SM43','SM44');
printFuture('Stellar Miracle (JP)', $FUT_SM);

$FUT_SCR = array('SCR1','SCR2','SCR3','SCR4','SCR5');
printFuture('Stellar Crown', $FUT_SCR);

$FUT_PD = array('PD1','PD2','PD3','PD4','PD5','PD6','PD7','PD8','PD9','PD10');
printFuture('Paradise Dragona (JP)', $FUT_PD);

$FUT_SV9EN = array('SV9EN1','SV9EN2','SV9EN3','SV9EN4');
printFuture('Scarlet & Violet 9 (TBA)', $FUT_SV9EN);

$FUT_SVPEN = array('SVPEN1','SVPEN2','SVPEN3','SVPEN4','SVPEN5','SVPEN6','SVPEN7','SVPEN8');
printFuture('Scarlet & Violet Promo', $FUT_SVPEN);

$FUT_SVP = array('SVP1','SVP2','SVP3','SVP4','SVP5');
printFuture('Scarlet & Violet Promo (JP)', $FUT_SVP);

$FUT_SVG = array('SVG1','SVG2');
printFuture('Special Deck Set ex Venusaur & Charizard & Blastoise (JP)', $FUT_SVG);

$FUT_SVJ = array('SVJ1','SVJ2');
printFuture('Battle Master Decks (JP)', $FUT_SVJ);

$FUT_SVL = array('SVL1','SVL2','SVL3','SVL4');
printFuture('Starter Set Tera Type Stellar (JP)', $FUT_SVL);

$FUT_KO = array('KO1');
printFuture('Korean Cards', $FUT_KO);

$FUT_CH = array('CH1','CH2','CH3','CH4','CH5','CH6','CH7','CH8');
printFuture('Chinese Cards', $FUT_CH);

$FUT_All = array('SM1','SM2','SM3','SM4','SM5','SM6','SM7','SM8','SM9','SM10','SM11','SM12','SM13','SM14','SM15','SM16','SM17','SM18','SM19','SM20','SM21','SM22','SM23','SM24','SM25','SM26','SM27','SM28','SM29','SM30','SM31','SM32','SM33','SM34','SM35','SM36','SM37','SM38','SM39','SM40','SM41','SM42','SM43','SM44','SCR1','SCR2','SCR3','SCR4','SCR5','PD1','PD2','PD3','PD4','PD5','PD6','PD7','PD8','PD9','PD10','SV9EN1','SV9EN2','SV9EN3','SV9EN4','SVPEN1','SVPEN2','SVPEN3','SVPEN4','SVPEN5','SVPEN6','SVPEN7','SVPEN8','SVP1','SVP2','SVP3','SVP4','SVP5','SVG1','SVG2','SVJ1','SVJ2','SVL1','SVL2','SVL3','SVL4','KO1','CH1','CH2','CH3','CH4','CH5','CH6','CH7','CH8');
printFuture('All', $FUT_All);

?>
